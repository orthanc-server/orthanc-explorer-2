...
export default {
  ...
  computed: {
    ...
    // 🆕 НОВЫЕ computed для карточного режима - НАЧАЛО
    isCardView() {
      // На мобильном в портретном режиме всегда CardView
      if (this.isMobilePortrait) return true;
      return this.viewMode === 'cards';
    },
    isTableView() {
      // На мобильном в портретном режиме никогда TableView
    ...
    sortedStudiesForCards() {
      ...
      // Создаём Map для O(1) lookup вместо O(n) find()
    ...
    // 🆕 НОВЫЕ computed для карточного режима - КОНЕЦ
  ...
  methods: {
    checkMobilePortrait() {
      ...
      // Если переключились в мобильный портрет - принудительно переключаем на CardView
    ...
    clearActiveModalityFilter() {
      // Сбрасываем активный фильтр модальностей из карточки
      this.activeModalityFilter = null;

      // Сбрасываем все модальности в true (как при инициализации)
    ...
    // Этот метод оставьте как был изначально (для кнопки Clear Filters)
    clearModalityFilter() {
    ...
    onFilterModality(modality) {
      ...
      // Обновляем флаги all/none
      this.allModalities = false;
      this.noneModalities = false;

      // Получаем строку фильтра для API
      ...
      // Обновляем URL и перезагружаем список
      this.updateUrlNoReload();
      this.reloadStudyList();
    },
    updateSelectAll() {
    ...
    emptyFilterForm() {
      ...
      // Сбрасываем быстрый фильтр карточного вида
    ...
    async clearFiltersUi() {
      ...
      // Сбрасываем активный фильтр модальностей
      this.activeModalityFilter = null;

      // Сбрасываем быстрый фильтр карточного вида и связанные с ним поля даты
      this.cardViewQuickFilter = 'all';
      this.filterStudyDateForDatePicker = null;
    ...
    visibilityChanged(isVisible, entry) {
      if (isVisible) {
        ...
        // В карточном режиме проверяем sortedStudiesForCards
    ...
    setCardQuickFilter(filter) {
      // Устанавливаем флаг, что обновление идёт программно
      this.updatingFilterUi = true;
      ...
      // Обновляем значения БЕЗ триггера watcher'ов
      ...
      // Снимаем флаг и делаем один запрос
      this.updatingFilterUi = false;

      // Единственное обновление
  ...
  watch: {
    // --- Роутинг: обновление фильтров при смене URL ---
    "$route"(to, from) {
      if (this.updatingRouteWithoutReload) {
        return;
      }
    ...
    // --- Модальности: deep watch для чекбоксов ---
    filterModalities: {
      deep: true,
      handler(newValue) {
        if (this.updatingFilterUi || this.initializingModalityFilter) {
    ...
    // --- Обновление "Select All" чекбокса при изменении выделения ---
    selectedStudiesIds() {
      this.updateSelectAll();
    },
    ...
    // --- Generic tags (search-as-you-type для текстовых полей) ---
    filterGenericTags: {
      deep: true,
      handler(newValue, oldValue) {
        if (this.updatingFilterUi) {
        ...
        // Для каждого изменённого тега вызываем updateFilter с задержкой
    ...
    // --- Когда конфигурация загружена, инициализируем фильтры ---
  ...
  // ==========================================================================
  created() {
    // Начальная проверка мобильного режима
    this.checkMobilePortrait();
  },

  mounted() {
    // Слушатель событий мобильного портретного режима (от App.vue или другого родителя)
    window.addEventListener("mobile-portrait-mode", this.handleMobilePortraitMode);
    window.addEventListener("resize", this.checkMobilePortrait);
    window.addEventListener("orientationchange", this.checkMobilePortrait);

    // Если конфигурация уже загружена к моменту mount — инициализируем
    if (this.isConfigurationLoaded) {
      this.initModalityFilter();
      this.translateDatePicker();
  ...
  beforeUnmount() {
    ...
    // Очистка всех таймеров search-as-you-type
